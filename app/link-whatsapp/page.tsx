// app/link-whatsapp/page.tsx
// Landing page for the WhatsApp-bot magic link (see
// app/api/whatsapp/webhook/[companyId]/route.ts, which sends this URL to an
// unrecognized WhatsApp sender). Requires a real Diract login -- that's the
// actual identity-proving step; app/api/whatsapp/bot/link/route.ts does the
...
export default function LinkWhatsAppPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!session) {
        const redirectTo = `/link-whatsapp?code=${encodeURIComponent(code ?? "")}`;
  ...
  const confirmLink = async () => {
    ...
    setLinking(true);
    setError(null);
    const res = await fetch("/api/whatsapp/bot/link", {
      method: "POST",
        ...
        {linked ? (
          <>
            ...
            <p className="text-[12px] text-slate-500">
              Your WhatsApp account is now linked to Diract. Go back to WhatsApp and send your question again.
            </p>
          </>
        ) : (
          <>
            <p className="text-[14px] font-bold text-slate-800">Link your WhatsApp account</p>
            <p className="text-[12px] text-slate-500">
              This lets the AI assistant remember your conversation across messages in WhatsApp, tied to your Diract account.
            </p>
            {error && (
